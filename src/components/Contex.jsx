...
class PizzeProvider extends Component {
  ...
  addTotals = () => {
    let CartsubTT = 0;
    this.state.pizzas
      .filter((pizza) => pizza.inCarta === true)
      .map((item) => (CartsubTT += item.total));
    let tempTax = 0;
    if (CartsubTT <= 60) {
  ...
  SortPizzaByPrice = (Sortby) => {
    const arraySort = require("array-sort");
    const numArray = [...this.state.pizzas];
    let NewSortPizzas = arraySort(numArray , Sortby , {reverse: this.state.sortReverse});
    this.setState({
        sortReverse: !this.state.sortReverse,
        pizzas:NewSortPizzas
    })

    // let newnum = numArray.sort((a, b) => {
    ...
    return (
      <PizzeContext.Provider
        value={{
          ...
          SortPizzaByPrice: this.SortPizzaByPrice,
        }}
      >
        {this.props.children}
      </PizzeContext.Provider>
